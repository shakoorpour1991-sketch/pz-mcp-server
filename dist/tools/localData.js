import { ParseGameFilesSchema, IndexKnowledgeBaseSchema, SearchKnowledgeBaseSchema, ListKnowledgeTopicsSchema, } from "../schemas.js";
import { formatKbIndexResults, formatKbSearchResults, formatKbTopics, formatParseResults, } from "../utils/formatters.js";
import { ErrorCode, McpError } from "@modelcontextprotocol/sdk/types.js";
import { knowledgeBasePath } from "../utils/config.js";
export const localDataTools = [
    {
        ...
        handler: async (args, ctx) => {
            ...
            if (gamePath) {
                try {
                    ...
                    throw new McpError(ErrorCode.InvalidParams, `Invalid gamePath: ${err.message}`);
                }
            }
            else {
            ...
            if (!detectedPath) {
                throw new McpError(ErrorCode.InvalidParams, "Could not detect Project Zomboid installation. Please provide the game path manually.");
            }
            const results = await ctx.parser.parseGameFiles(detectedPath, forceReparse);
            return {
                content: [
                    {
                        type: "text",
    ...
    },
    {
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                throw new McpError(ErrorCode.InvalidParams, `Invalid knowledge base path: ${err.message}`);
            }
            const result = await ctx.knowledgeBaseManager.indexDirectory(resolvedPath, {
                overwrite,
            });
            return {
                content: [
    ...
    },
    {
        ...
        handler: async (args, ctx) => {
            ...
            const results = await ctx.knowledgeBaseManager.search(query, topic ? { topic, limit } : { limit });
            return {
                content: [
                    {
                        type: "text",
