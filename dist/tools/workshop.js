...
import { WorkshopSearchSchema, WorkshopGetDetailsSchema, WorkshopDownloadSchema, WorkshopAnalyzeSchema, } from "../schemas.js";
import { formatWorkshopDetails, formatWorkshopDownload, formatWorkshopModReport, formatWorkshopSearchResults, } from "../utils/formatters.js";
import { parseWorkshopInput, PZ_APPID, } from "../workshop/SteamWorkshopClient.js";
...
function enforceDownloadSizeLimit(fileSize) {
    const max = maxDownloadBytes();
    if (fileSize > 0 && fileSize > max) {
        throw new McpError(ErrorCode.InvalidParams, `Item is ${formatBytes(fileSize)}, which exceeds the configured download limit of ${formatBytes(max)}. Raise PZ_MCP_MAX_DOWNLOAD_BYTES to allow it.`);
    }
}
...
export const workshopTools = [
    {
        ...
        description: "Browse the Project Zomboid Steam Workshop (AppID 108600) by text. Best-effort: uses the public community browse page (keyless). Paste a workshop URL/id with workshop_get_details for guaranteed resolution",
    ...
    },
    {
        ...
        description: "Resolve full metadata for a Project Zomboid workshop item from its id or steamcommunity URL (Steam Web API, keyless, 24h cache)",
    ...
    },
    {
        ...
        description: "Download a Project Zomboid workshop item via SteamCMD into the workshop workspace dir (PZ_WORKSHOP_DIR or <Steam>/steamapps/workshop/content/108600). Verifies the item belongs to Project Zomboid first. Requires steamcmd (STEAMCMD_PATH or common locations)",
        ...
        handler: async (args, ctx) => {
            ...
            if (ctx.activeWorkshopDownloads.has(resolvedId)) {
                throw new McpError(ErrorCode.InvalidParams, `Item ${resolvedId} is already downloading — wait for it to finish or pause it first.`);
            }
            ...
            const isPz = details.appId === PZ_APPID;
            if (!isPz) {
                throw new McpError(ErrorCode.InvalidParams, `Item ${resolvedId} belongs to app ${details.appId || "unknown"}, not Project Zomboid (${PZ_APPID}). Refusing to download.`);
            }
            ...
            if (dryRun) {
                ...
                const alreadyPresent = targetDir !== "" && existsSync(join(targetDir, resolvedId));
                const sizeNote = details.fileSize > 0
            ...
            try {
                let lastPct = -1;
                const result = await ctx.steamCmdDownloader.download(resolvedId, (phase) => logger.info({ workshopId: resolvedId }, "workshop_download: %s", phase), {
                    ...
                    onProgress: (info) => {
                        const pct = info.expectedBytes > 0 ? Math.min(99, Math.round(info.pct)) : 0;
                        if (pct === lastPct)
                            return;
                        lastPct = pct;
                        logger.info({ workshopId: resolvedId }, "workshop_download: progress %d%% (%d/%d bytes)", pct, info.bytes, info.expectedBytes);
                    },
                });
                return {
                    content: [
    ...
    },
    {
        ...
        description: "Fetch & Analyze a Project Zomboid workshop item: download via SteamCMD, parse its scripts into the DB, run analyze_mod + reference checks, and return a full Mod Report (what it adds, quality score, issues, recommendations)",
        ...
        handler: async (args, ctx) => {
            ...
            const isPz = details.appId === PZ_APPID;
            if (!isPz) {
                throw new McpError(ErrorCode.InvalidParams, `Item ${resolvedId} belongs to app ${details.appId || "unknown"}, not Project Zomboid (${PZ_APPID}). Refusing to analyze.`);
            }
            enforceDownloadSizeLimit(details.fileSize);
            const onPhase = (phase) => logger.info({ workshopId: resolvedId }, "workshop_analyze: %s", phase);
            ...
            try {
                ...
                const parseResults = await wsParser.parseModDirectory(dl.downloadedPath);
