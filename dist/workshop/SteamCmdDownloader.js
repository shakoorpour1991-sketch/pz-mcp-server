...
import { existsSync, mkdirSync, rmSync, renameSync, readdirSync, lstatSync, statfsSync, mkdtempSync, } from "fs";
import { dirname, join, resolve, sep } from "path";
import { homedir } from "os";
import { PZ_APPID } from "./SteamWorkshopClient.js";
import { PathManager } from "../utils/PathManager.js";
import logger from "../utils/logger.js";
const COMMON_STEAMCMD_PATHS = {
    win32: [
...
export class SteamCmdDownloader {
    ...
    constructor(opts = {}) {
        ...
        this.runner =
            opts.runner ??
                ((cmd, args) => new Promise((resolveRun, reject) => {
    ...
    async resolveSteamCmdPath() {
        ...
        throw new Error("SteamCMD not found. Install it (https://developer.valvesoftware.com/wiki/SteamCMD) and either set STEAMCMD_PATH, drop steamcmd.exe in C:\\steamcmd, or place it in your Steam directory.");
    }
    /** Resolve the workshop output dir (env → Steam install → error). */
    async resolveWorkshopDir() {
        if (this.workshopDir)
            return this.workshopDir;
        ...
        throw new Error("Could not determine the workshop content directory. Set PZ_WORKSHOP_DIR to the folder where mods should be downloaded (e.g. D:\\Steam\\steamapps\\workshop\\content\\108600).");
    }
    /**
     ...
     */
    async download(id, onPhase, opts = {}) {
        if (!/^\d{6,15}$/.test(id)) {
        ...
        if (!resolvedDest.startsWith(resolvedWorkshopDir + sep) &&
        ...
        if (expectedBytes > 0) {
            try {
                ...
                if (free < expectedBytes + margin) {
                    throw new Error(`Not enough free disk space on ${dirname(workshopDir)}: need ~${formatSize(expectedBytes + margin)} free, only ${formatSize(free)} available.`);
                }
            }
            catch (err) {
                ...
                logger.warn("disk-space probe failed (continuing): %s", err instanceof Error ? err.message : String(err));
            }
        }
        ...
        try {
            ...
            const dlDir = join(tempDir, "steamapps", "workshop", "content", PZ_APPID, id);
            ...
            for (;;) {
                ...
                if (attempts > maxAttempts) {
                    throw new Error(`SteamCMD failed ${maxAttempts}× with transient exit code 7. Try again later.`);
                }
                if (attempts > 1) {
                    logger.warn({ id, attempt: attempts }, "retrying steamcmd (previous exit code 7)");
                ...
                if (code === 0 || parsed.downloaded) {
                    const src = join(tempDir, "steamapps", "workshop", "content", PZ_APPID, id);
                    if (!existsSync(src)) {
                        throw new Error(`SteamCMD reported success but content was not found at ${src}.`);
                    }
            ...
            if (tempDir) {
                try {
                    ...
                    logger.warn({
                        tempDir,
                        err: cleanupErr instanceof Error
                            ? cleanupErr.message
                            : String(cleanupErr),
    ...
    loginArgs() {
        ...
        const useCreds = process.env.STEAMCMD_USE_CREDENTIALS === "1" ||
            process.env.STEAMCMD_USE_CREDENTIALS === "true";
        if (user) {
            if (!useCreds) {
                throw new Error("STEAMCMD_USER is set but STEAMCMD_USE_CREDENTIALS is not enabled. " +
            ...
            if (!pass) {
                throw new Error("STEAMCMD_USER is set but STEAMCMD_PASS is empty — +login would fail with an empty password. Set both, or unset STEAMCMD_USER to use anonymous.");
            }
    ...
    parseOutput(output) {
        const success = output.match(/Success\. Downloaded item \d+ to "([^"]+)" \(\s*(\d+)\s*bytes\)/) || output.match(/Download item \d+ result: OK/);
        if (success) {
            const noteMatch = output.match(/Success\. Downloaded item [^"]+ to "([^"]+)"/);
            return {
                downloaded: true,
        ...
        if (/No subscription/i.test(output)) {
            return {
                downloaded: false,
                bytes: 0,
                error: new Error("SteamCMD (anonymous) was rejected for this item — it requires an account that owns a subscription. Subscribe in Steam, or set STEAMCMD_USER/STEAMCMD_PASS (never on the command line)."),
            };
        }
        if (/Invalid workshop item|item not found|doesn't exist/i.test(output)) {
            return {
                downloaded: false,
                bytes: 0,
                error: new Error(`SteamCMD could not find workshop item (invalid id or private item).`),
            };
        }
...
//# sourceMappingURL=SteamCmdDownloader.js.map