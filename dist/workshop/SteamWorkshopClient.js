 ...
 */
export function parseWorkshopInput(input) {
    ...
    try {
        ...
        throw new Error(`Could not parse workshop id from "${raw}". Paste a workshop URL (https://steamcommunity.com/sharedfiles/filedetails/?id=…) or a numeric item id.`);
    }
...
export class SteamWorkshopClient {
    ...
    constructor(opts = {}) {
        this.cacheFile =
            opts.cacheFile ?? join(dataDir(), "workshop_metadata.json");
    ...
    async search(query, limit) {
        ...
        if (items.length === 0) {
            ...
            if (!hasFileDetails && !hasBrowseEvidence) {
                throw new Error(`No items parsed from Steam for "${q}" (page may be down, blocked, or age-gated). ` +
    ...
    parseBrowseHtml(html, limit) {
        ...
        for (const [id, idxs] of occurrences) {
            ...
            for (const idx of idxs) {
                ...
                if (!title) {
                    const am = after.match(new RegExp(`<a[^>]*filedetails/\\?id=${id}[^>]*>([\\s\\S]*?)</a>`));
                    if (am) {
            ...
            if (!title) {
                for (const idx of idxs) {
                    const alt = first(html.slice(Math.max(0, idx - 150), idx + 300), /<img[^>]*alt="([^"]+)"/);
                    if (alt) {
            ...
            if (!title || /^learn\s+more$/i.test(title))
                continue;
            // Author: the "By NAME" link lives adjacent to the title link.
            const author = cleanHtml(first(html.slice(Math.max(0, titleIdx - 300), titleIdx + 900), /<a[^>]*\/myworkshopfiles\/[^>]*>\s*By\s*([^<]+)</i));
            items.push({
                id,
                title,
                author: author || "",
    ...
    loadCache() {
        try {
            ...
            logger.warn("Workshop metadata cache unreadable (ignoring): %s", err instanceof Error ? err.message : String(err));
        }
        return { entries: {} };
    }
    saveCache() {
        try {
            ...
            writeFileSync(tmpFile, JSON.stringify(this.cache, null, 2), {
                mode: 0o600,
            });
            renameSync(tmpFile, this.cacheFile);
        }
        catch (err) {
            logger.warn("Could not write workshop metadata cache: %s", err instanceof Error ? err.message : String(err));
        }
    }
    /* ---------------- shared ---------------- */
    async request(url, init = {}) {
        ...
        try {
            ...
            if (err instanceof Error && err.name === "AbortError") {
                throw new Error(`Steam request timed out after ${this.timeoutMs / 1000}s. Is the network up?`);
            }
            throw err;
        }
        finally {
...
//# sourceMappingURL=SteamWorkshopClient.js.map