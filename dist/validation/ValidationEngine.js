...
export class ValidationEngine {
    ...
    async validateScript(content, expectedType, strict = false) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
    ...
    async checkReferences(references, type = "all") {
        const results = [];
    ...
    parseProperty(line, properties, blockType) {
        if (blockType === "fixing") {
    ...
    async validateBlock(block, result, strict) {
        // Check required properties
        const required = this.requiredProperties[block.type];
        if (required) {
    ...
    async validateProperty(key, value, block, result, strict) {
    ...
    async validateBlockSpecific(block, result, _strict) {
        switch (block.type) {
            case "item":
    ...
    validateItemBlock(block, result) {
    ...
    async validateRecipeBlock(block, result) {
    ...
    validateFixingBlock(block, result) {
    ...
    validateSoundBlock(block, result) {
    ...
    validateEvolvedRecipeBlock(block, result) {
    ...
    validateSyntax(content, result) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const { code, inBlockComment: newState } = stripLineComments(lines[i], inBlockComment);
