...
const BLOCK_RE = /^(item|recipe|craftRecipe|craftrecipe|evolvedrecipe|fixing|sound|vehicle|entity|mod|model|event|timedAction|fluid|physics|mannequin|clock|energy|animation|bodylocation|creature)\s+([^{]+)/;
/** Strip line comments and inline block comments, tracking block-comment state. */
export function stripLineComments(line, inBlockComment) {
    let code = "";
    let i = 0;
    while (i < line.length) {
        if (inBlockComment) {
...
export function countBraces(code) {
    return {
        open: (code.match(/\{/g) || []).length,
        close: (code.match(/\}/g) || []).length,
    };
}
export function scanScriptBlocks(content, defaultModule = "Base") {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const { code: stripped, inBlockComment: newState } = stripLineComments(rawLine, inBlockComment);
        ...
        if (blockMatch &&
            ...
            currentBlock = {
                type: rawType === "craftRecipe" || rawType === "craftrecipe"
                    ? "recipe"
                    : rawType,
                rawType,
