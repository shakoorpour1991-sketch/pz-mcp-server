...
export const REQUIRED_MOD_FILES = ["mod.info"];
...
export class ModWorkspaceManager {
    ...
    validateWorkspacePath(inputPath) {
        ...
        if (!withinWorkspace && this.config.strictPaths) {
            throw new Error(`Path '${resolved}' is not within any configured workspace root: ${this.config.workspaceRoots.join(", ")}`);
        }
        return resolved;
    }
    /**
     ...
     */
    async createMod(options, targetDir) {
        try {
            ...
            logger.error(`Failed to create mod: ${err.message}`);
     ...
     */
    generatePlaceholderScript(module, modId) {
...
`;
    ...
    async inspectMod(modPath) {
        ...
        structure.hasCommonFolder = layouts.some((l) => l.scriptsDirs.some((d) => d.includes("/common/") || d.includes("\\common\\")) ||
        ...
        if (structure.scriptCount > 0)
            structure.detectedContentTypes.push("scripts");
        if (structure.luaCount > 0)
            structure.detectedContentTypes.push("lua");
        if (structure.assetCount > 0)
            structure.detectedContentTypes.push("assets");
        return structure;
    }
    /**
     * Recursively scan files in mod directory.
     */
    async scanFiles(dirPath, modRoot, structure) {
        try {
            ...
            for (const entry of entries) {
                ...
                if (entryStat.isDirectory()) {
                    ...
                    let fileType = "other";
                    if (MOD_FILE_EXTENSIONS.scripts.includes(ext)) {
                        fileType = "script";
                        structure.scriptCount++;
                    }
                    else if (MOD_FILE_EXTENSIONS.lua.includes(ext)) {
                        fileType = "lua";
                        structure.luaCount++;
                    }
                    else if (MOD_FILE_EXTENSIONS.assets.includes(ext)) {
                        fileType = "asset";
                        structure.assetCount++;
                    }
                    else if (MOD_FILE_EXTENSIONS.data.includes(ext)) {
                        fileType = "data";
                    }
                    else if (MOD_FILE_EXTENSIONS.docs.includes(ext)) {
                        fileType = "doc";
                    }
            ...
            logger.warn(`Failed to scan directory ${dirPath}: ${err.message}`);
        }
    }
    /**
     * Parse mod.info file.
        ...
        for (const infoPath of modInfoPaths) {
            if (existsSync(infoPath)) {
                try {
                    ...
                    logger.warn(`Failed to parse mod.info at ${infoPath}: ${err.message}`);
                }
            }
        }
        ...
        for (const layout of layouts) {
            if (layout.modInfoPath) {
                try {
                    ...
                    logger.warn(`Failed to read mod.info id: ${err.message}`);
                }
            }
        }
        return undefined;
     ...
     */
    async listFiles(modPath, options) {
        const validatedPath = this.pathManager.validateInputPath(modPath, "dir");
     ...
     */
    async readFile(modPath, filePath) {
        try {
     ...
     */
    async writeFile(modPath, filePath, content, options) {
        try {
            const validatedModPath = this.pathManager.validateInputPath(modPath, "dir");
            // Ensure filePath doesn't escape mod directory
            const normalizedFile = normalize(filePath);
            ...
            const result = { success: true, path: fullPath };
            if (backupPath) {
     ...
     */
    async deleteFile(modPath, filePath, options) {
        try {
            const validatedModPath = this.pathManager.validateInputPath(modPath, "dir");
            const normalizedFile = normalize(filePath);
            if (normalizedFile.startsWith("..") || isAbsolute(normalizedFile)) {
     ...
     */
    async renameFile(modPath, oldPath, newPath, _options) {
        try {
            const validatedModPath = this.pathManager.validateInputPath(modPath, "dir");
     ...
     */
    async getProjectStatus(modPath) {
        const validatedPath = this.pathManager.validateInputPath(modPath, "dir");
        const structure = await this.inspectMod(validatedPath);
     ...
     */
    async patchFile(modPath, filePath, patchFn, options) {
...
//# sourceMappingURL=ModWorkspaceManager.js.map