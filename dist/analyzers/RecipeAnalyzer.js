 ...
 */
import { referenceCandidates, } from "../database/DatabaseManager.js";
/**
...
export class RecipeAnalyzer {
    ...
    producersOf(id) {
        ...
        for (const p of idx.refProducers.get(id) ?? [])
            out.add(p);
        return out;
    }
    /**
    ...
    consumersOf(id) {
        ...
        for (const c of idx.refConsumers.get(id) ?? [])
            out.add(c);
        for (const t of idx.itemTags.get(id) ?? []) {
            for (const c of idx.tagIngestors.get(t) ?? [])
                out.add(c);
        }
        return out;
    }
    async cachedResolveItem(id) {
        ...
        if (kind === "recipe") {
            ...
            const addIng = (id, count, tag) => {
                ...
                if (existing) {
                    ...
                    if (tag && !existing.tag)
                        return; // direct entry already wins
                    if (!tag && existing.tag) {
            ...
            const tools = (this.idx?.toolsByRecipe.get(id) ?? []).map((t) => (t.count > 0 ? { id: t.id, count: t.count } : { id: t.id }));
            ...
            if (meta) {
                ...
                if (meta.skillLevel !== undefined)
                    out.skillLevel = meta.skillLevel;
                if (tools.length > 0)
                    out.tools = tools;
                node.meta = out;
            }
     ...
     */
    async analyzeChain(seed, direction = "both", maxDepth = 3, options = {}) {
        ...
        if (options.target) {
            ...
            for (const nid of path) {
                ...
                if (existing) {
                    ...
                    const kind = (await this.resolveItem(nid))?.info.type === "recipe"
                        ? "recipe"
                        : "item";
    ...
    async walk(seedId, seedKind, direction, maxDepth) {
        const nodes = [];
        const cycles = [];
        const visited = new Set();
        const queue = [{ id: seedId, kind: seedKind, depth: 0 }];
        ...
        while (head < queue.length) {
            ...
            if (node.cycle && kind === "recipe") {
                const loopItem = node.ingredients.find((i) => node.results.some((r) => r.id === i.id));
                if (loopItem)
                    cycles.push({ recipe: id, item: loopItem.id });
            }
            nodes.push(node);
     ...
     */
    shortestPath(nodes, startId, targetId) {
        if (startId === targetId)
            return [startId];
     ...
     */
    async detectConflicts(limit = 50) {
        ...
        for (const row of tagRows) {
            if (seenItems.has(row.tag))
                continue; // exact duplicate wins the ranking
...
//# sourceMappingURL=RecipeAnalyzer.js.map