...
export class ModAnalyzer {
    ...
    async analyzeMod(modPath, options = {}) {
        const { checkBalance = true, checkCompatibility = true, checkPerformance = true, strictValidation = false, } = options;
    ...
    async analyzeStructure(modPath) {
        const structure = {
            hasModInfo: false,
        ...
        const hasRootModInfo = existsSync(join(modPath, "mod.info")) ||
        ...
        structure.hasCommonFolder =
            existsSync(commonPath) ||
                layouts.some((l) => l.scriptsDirs.some((d) => d.includes(`${sep}common${sep}`)) ||
    ...
    isExpectedTopLevelEntry(entry, isDir) {
        if (entry.startsWith("."))
            return true;
        if (entry === "mod.info")
            return true;
        if (entry === "common")
            return isDir; // B42 common/ folder
        if (isDir && /^\d+(\.\d+)*$/.test(entry))
            return true; // 42 / 41.78
        if (entry === "media")
            return isDir; // legacy root layout
        if (entry === "mods")
            return isDir; // workshop pack container
        ...
        // (mod-analyzer review).
        if (/^readme(\.md|\.txt)?$/i.test(entry))
            return true;
        if (entry === "workshop.txt")
            return true;
        return false;
    }
    /**
     ...
     */
    async parseModItems(modPath) {
        if (this.parsedItemsCache?.modPath === modPath) {
            return this.parsedItemsCache.items;
        }
        const tempPath = join(tmpdir(), `pz-analyze-${process.pid}-${Date.now()}.db`);
    ...
    async countFiles(dirPath, structure) {
        try {
            ...
            logger.warn(`Failed to read directory ${dirPath} during file count: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async parseModInfo(modPath) {
    ...
    async analyzeScripts(modPath, result, strict) {
    ...
    async analyzeScriptDirectory(dirPath, result, strict) {
        try {
            const entries = await readdir(dirPath);
            for (const entry of entries) {
                const fullPath = join(dirPath, entry);
    ...
    async analyzeScriptFile(filePath, result, strict) {
        try {
    ...
    async analyzeLuaFiles(modPath, result) {
        const layouts = await this.getLayouts(modPath);
        const seen = new Set();
        for (const layout of layouts) {
    ...
    async analyzeLuaDirectory(dirPath, result) {
        try {
            const entries = await readdir(dirPath);
            for (const entry of entries) {
                const fullPath = join(dirPath, entry);
    ...
    async analyzeLuaFile(filePath, result) {
        try {
            const content = await readFile(filePath, "utf-8");
    ...
    stripLuaComments(content) {
        ...
        (function (State) {
            ...
            State[State["SingleString"] = 1] = "SingleString";
            State[State["DoubleString"] = 2] = "DoubleString";
            State[State["LongString"] = 3] = "LongString";
            State[State["LineComment"] = 4] = "LineComment";
            State[State["BlockComment"] = 5] = "BlockComment";
        })(State || (State = {}));
    ...
    checkLuaSyntax(content, filePath, result) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes("Events.OnPreMapLoad.Add")) {
                result.issues.push({
                    ...
                    line: i + 1,
                    severity: "info",
                    message: "Events.OnPreMapLoad is not used by B42 vanilla — prefer Events.OnGameBoot.Add for initialization",
                    code: "LUA_BEST_PRACTICE",
                });
            }
        }
    }
    checkDeprecatedAPI(content, filePath, result) {
    ...
    checkSemanticIssues(content, filePath, result) {
        ...
        const blockOpenerCount = (blanked.match(/\bif\b/g) || []).length +
            ...
            standaloneDo;
        const closerCount = (blanked.match(/\bend\b/g) || []).length +
    ...
    async analyzeBalance(modPath, _result) {
        const balance = {
            itemCount: 0,
            averageStats: {},
            outliers: [],
    ...
    async findBalanceOutliers(modItems, balance) {
        for (const category of ModAnalyzer.BALANCE_CATEGORIES) {
            ...
            const vanillaItems = await this.db.getItemsByPropertyType(category.type, 10000);
            ...
            for (const stat of category.stats) {
                ...
                const variance = values.reduce((sum, val) => sum + (val - mean) ** 2, 0) /
                    values.length;
            ...
            for (const item of modItems) {
                ...
                for (const stat of category.stats) {
                    ...
                    if (typeof value !== "number" || baseline === undefined)
                        continue;
    ...
    calculateBalanceScore(balance) {
        ...
        const extremeOutliers = balance.outliers.filter((o) => o.ratio !== undefined && o.ratio >= 10);
        score -= extremeOutliers.length * 15;
        return Math.max(0, score);
    }
    generateBalanceRecommendations(balance) {
     ...
     */
    checkPerformanceIssues(content, filePath, result) {
        const blanked = this.blankLuaStrings(content);
        const lines = blanked.split("\n");
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.includes("Events.OnPlayerUpdate.Add")) {
                result.issues.push({
                    ...
                    severity: "info",
                    message: "Per-frame handler registered (Events.OnPlayerUpdate) — keep the body cheap or use a periodic timer (EveryOneMinute/EveryHours)",
                    code: "PERF_ISSUE",
                });
            }
                ...
                result.issues.push({
                    ...
                    line: i + 1,
                    severity: "warning",
                    message: "Events.OnTick runs every game tick — a server-side tick handler should be cheap or use a periodic timer",
                    code: "PERF_ISSUE",
                });
            }
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = i + 1; j < Math.min(lines.length, i + 10); j++) {
                ...
                if (body.trim() === "")
                    continue;
                if (!/^\s/.test(body))
                    break; // dedented → loop body ended
                if (listenerRe.test(body)) {
                    result.issues.push({
                        file: filePath,
                        line: i + 1,
                        severity: "warning",
                        message: "Event listener registered inside a loop — move the registration outside the loop",
                        code: "PERF_ISSUE",
                    });
                    break;
                }
    ...
    async analyzeCompatibility(modPath, result) {
        const compatibility = {
            conflicts: [],
            missingDependencies: [],
            incompatibleMods: [],
        ...
        try {
            ...
            for (const item of modItems.slice(0, 200)) {
                ...
                if (other) {
                    ...
                    if (compatibility.conflicts.length >= 50)
                        break;
                }
            }
        }
    ...
    async analyzePerformance(modPath, result) {
        const performance = {
            largeFiles: [],
            recommendations: [],
        };
        await this.findLargeFiles(modPath, performance);
        this.generatePerformanceRecommendations(performance, result.structure.assetCount);
        return performance;
    }
    async findLargeFiles(dirPath, performance) {
        try {
            const entries = await readdir(dirPath);
            for (const entry of entries) {
                const fullPath = join(dirPath, entry);
            ...
            // Directory read error — no longer swallowed silently (freebuff review
            // §3 code smell #5).
            logger.warn(`Failed to read directory ${dirPath} during large-file scan: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    generatePerformanceRecommendations(performance, assetCount) {
        ...
        if (assetCount > 100) {
            performance.recommendations.push(`Mod bundles ${assetCount} asset files — large texture/audio packs slow loading; consider compressing or splitting optional assets`);
        }
    }
    calculateQualityMetrics(result) {
        ...
        if (result.structure.hasModInfo)
            metrics.structure += 30;
        if (result.structure.hasCorrectStructure)
            metrics.structure += 40;
        if (result.structure.scriptCount > 0)
            metrics.structure += 20;
        if (result.structure.missingFiles.length === 0)
        ...
        if (result.modInfo?.name)
            metrics.documentation += 10;
        if (result.modInfo?.id)
            metrics.documentation += 10;
        if (result.modInfo?.description)
            metrics.documentation += 20;
        if (result.modInfo?.author)
            metrics.documentation += 10;
        if (result.modInfo?.version)
            metrics.documentation += 10;
        const hasReadme = existsSync(join(result.modPath, "README.md")) ||
            existsSync(join(result.modPath, "readme.md")) ||
            existsSync(join(result.modPath, "README.txt"));
        if (hasReadme)
            metrics.documentation += 20;
        // Overall score
        metrics.overall = Math.round((metrics.structure +
            metrics.syntax +
            metrics.balance +
    ...
    generateRecommendations(result) {
        ...
        if (result.structure.unexpectedFiles.length > 0) {
            recommendations.push(`Unexpected top-level files/folders: ${result.structure.unexpectedFiles.slice(0, 3).join(", ")}`);
        }
        if (!result.structure.hasCorrectStructure) {
...
//# sourceMappingURL=ModAnalyzer.js.map