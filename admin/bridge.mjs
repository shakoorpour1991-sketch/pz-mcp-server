...
function spawnChild() {
  ...
  const spawnOpts = {
    ...
    env: {
      ...
      ...(deckSettings.workshopDir
        ? { PZ_WORKSHOP_DIR: deckSettings.workshopDir }
        : {}),
  ...
  child.on("exit", (code) => {
    ...
    for (const [, p] of pending) {
      ...
      p.reject(new Error("server process exited"));
    }
    pending.clear();
...
function sendToServer(msg, timeout = 120000) {
  return new Promise((resolve, reject) => {
    if (!child || !child.stdin.writable)
      return reject(new Error("server process not running yet"));
    ...
    const timer = setTimeout(() => {
      pending.delete(msg.id);
      reject(
        new Error(`request #${msg.id} timed out after ${timeout / 1000}s`),
      );
    }, timeout);
...
function dbStats() {
  ...
  try {
    ...
    try {
      for (const r of db
        .prepare("SELECT type, COUNT(*) AS c FROM items GROUP BY type")
        .all()) {
        out.byType[r.type] = r.c;
        out.total += r.c;
      }
      out.references = db
        .prepare('SELECT COUNT(*) AS c FROM "references"')
        .get().c;
      ...
      if (!/no such table/i.test(msg))
        console.error(`[bridge] dbStats query failed: ${msg}`);
    }
    db.close();
    return out;
  } catch (err) {
    ...
    if (process.platform === "win32") {
      execFile(
        ...
        (err, out) => {
          ...
          resolve(
            m ? Math.round(parseInt(m[1].replace(/,/g, ""), 10) / 1024) : null,
          );
        },
      );
    } else {
      try {
        const m = readFileSync(`/proc/${child.pid}/status`, "utf8").match(
          /VmRSS:\s+(\d+)\s+kB/,
        );
...
function streamToFile(req, filePath, maxBytes) {
  return new Promise((resolve, reject) => {
    ...
    req.on("data", (c) => {
      total += c.length;
      if (total > maxBytes)
        return fail(new Error("upload exceeds the 8 GiB limit"));
      if (!out.write(c)) req.pause();
    });
    out.on("drain", () => req.resume());
    req.on("end", () => {
      if (!failed) out.end(() => resolve(total));
    });
    req.on("error", fail);
    out.on("error", fail);
  });
...
let schemaLoadMtime = 0;

/* Reload the tool schemas whenever dist/schemas.js changes on disk (checked
 * on every normalization). The bridge is long-running, so without this a
 * rebuild leaves the deck serving the old zod schema — fields that became
 * optional stay marked required until the whole dashboard is restarted. */
async function loadToolSchemas() {
  try {
    const { stat } = await import("node:fs/promises");
    const st = await stat(join(ROOT, "dist", "schemas.js"));
    if (st.mtimeMs === schemaLoadMtime && TOOL_SCHEMAS) return TOOL_SCHEMAS;
    schemaLoadMtime = st.mtimeMs;
  } catch {
    /* no dist yet */
  }
  try {
    ({ TOOL_SCHEMAS } = await import("../dist/schemas.js?t=" + Date.now()));
  } catch {
    try {
...
async function liveSchemaToJSONSchema(name) {
  const TOOL_SCHEMAS = await loadToolSchemas();
...
async function normalizeInputSchema(tool) {
  ...
  if (
    ...
    return raw; // already JSON Schema
  const live = tool.name && (await liveSchemaToJSONSchema(tool.name));
  if (live) return live;
...
async function normalizeToolsReply(reply) {
  const tools = reply && reply.result && reply.result.tools;
  if (Array.isArray(tools)) {
    for (const t of tools) {
      if (t && t.inputSchema) t.inputSchema = await normalizeInputSchema(t);
    }
  }
  return reply;
}
...
async function readBody(req) {
  ...
  for await (const chunk of req) {
    s += chunk;
    if (s.length > 5_000_000) throw new Error("body too large");
  }
  return s;
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && DECK_STATIC.has(url.pathname)) {
      res.writeHead(200, {
        "Content-Type": DECK_STATIC.get(url.pathname),
    ...
    if (req.method === "GET" && url.pathname === "/api/stats")
      return json(res, lastTelemetry || { state });
    ...
    if (req.method === "GET" && url.pathname === "/api/env") {
      return json(res, {
        ...
        workshopDir:
          readDeckSettings().workshopDir || process.env.PZ_WORKSHOP_DIR || null,
    ...
    if (req.method === "POST" && url.pathname === "/api/workshop-dir") {
      ...
      if (!p)
        return json(res, { ok: false, error: "Enter a folder path" }, 400);
      if (!/^[A-Za-z]:[\\/]/.test(p) && !/^\\\\/.test(p)) {
        return json(
          res,
          {
            ok: false,
            error: "Use an absolute path, e.g. D:\\PZ-Mods\\Workshop",
          },
          400,
        );
      }
      ...
      try {
        ...
        return json(
          res,
          { ok: false, error: `Folder not writable: ${p}` },
          400,
        );
      }
      const s = readDeckSettings();
    ...
    if (req.method === "POST" && url.pathname === "/api/open-workshop-dir") {
      if (process.platform !== "win32") {
        return json(
          res,
          { ok: false, error: "Open folder is only supported on Windows" },
          400,
        );
      }
      let dir =
      ...
      if (!dir) {
        return json(
          res,
          {
            ok: false,
            error:
              "Could not determine the workshop folder — set one in Settings → Workshop first",
          },
          400,
        );
      }
      ...
      execFile("explorer", [dir], () =>
        pushLog(`↗ opened workshop folder ${dir}`),
      );
    ...
    if (req.method === "POST" && url.pathname === "/api/mods-dir") {
      ...
      if (!/^[A-Za-z]:[\\/]/.test(p) && !/^\\\\/.test(p)) {
        return json(
          res,
          { ok: false, error: "Use an absolute path, e.g. D:\\PZ-Mods\\Mods" },
          400,
        );
      }
      try {
    ...
    if (req.method === "POST" && url.pathname === "/api/open-mods-dir") {
          ...
          const reply = await sendToServer(
            {
              ...
              id: Date.now(),
              method: "tools/call",
              params: { name: "detect_pz_paths", arguments: {} },
            },
            20000,
          );
      ...
      try {
        ...
        return json(
          res,
          { ok: false, error: `Folder not writable: ${dir}` },
          400,
        );
      }
    ...
    if (req.method === "POST" && url.pathname === "/api/mod-source/zip") {
      ...
      try {
        ...
        return json(
          res,
          { ok: false, error: "Upload failed: " + String(e.message || e) },
          400,
        );
      }
    }
    if (req.method === "POST" && url.pathname === "/api/mod-source/folder") {
      const batch = safeUploadName(
    ...
    if (req.method === "POST" && url.pathname === "/api/mod-source/cleanup") {
      ...
      if (!norm.startsWith(base + "/")) {
        return json(
          res,
          { ok: false, error: "Refusing cleanup outside the uploads folder" },
          400,
        );
      }
      try {
    ...
    if (req.method === "POST" && url.pathname === "/api/workshop/pause") {
      if (process.platform !== "win32") {
        return json(
          res,
          { ok: false, error: "Pause is only supported on Windows" },
          400,
        );
      }
      execFile("taskkill", ["/IM", "steamcmd.exe", "/T", "/F"], (err) => {
        ...
        if (err && !/not found|no running instance/i.test(msg)) {
          return json(
            res,
            { ok: false, error: "Could not pause: " + msg },
            500,
          );
        }
    ...
    if (req.method === "POST" && url.pathname === "/rpc") {
      ...
      if (msg.method === "tools/call" && msg.params && msg.params.name) {
        const bad = await validateArgs(msg.params.name, msg.params.arguments);
        if (bad)
          return json(
            res,
            {
      ...
      try {
        ...
        return reply === null
          ...
          : json(res, await normalizeToolsReply(reply));
      } catch (e) {
        return json(
          res,
          {
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  console.log(
    `  ▸ database    ${existsSync(DB_PATH) ? DB_PATH : "not created yet — run parse_game_files"}`,
  );
  console.log("");
});
spawnChild();
