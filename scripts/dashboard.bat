...
echo  Done? Press Ctrl+C in that window - or run:  dashboard.bat /?
echo.
endlocal
exit /b 0
...
echo    Option A - clean, recommended: in the "Glass Control Deck" console
...
echo    Option B - Task Manager: end the node.exe whose Command Line contains
echo       "bridge.mjs" (or the PID listening on 8787).
echo    Option C - emergency, if the console window is gone but port 8787 is
