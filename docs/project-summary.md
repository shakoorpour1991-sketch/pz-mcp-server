...
|---|---|---|
| `src/index.ts` | MCP server bootstrap: stdio transport, tool/prompt/resource registration (zod schemas), server version `1.1.0` | 9 tools, 4 prompts, `knowledge://` resources |
...
- `analyze_mod` — modPath, checkBalance (default true), checkCompatibility (true), generateReport (true)
...
- Repo root also holds Orca orchestration artifacts (SKILL.md, orchestration.md, PLAN.md) — not project docs.
