...
describe('pz-mcp-server integration', () => {
  ...
  test('tools/list exposes all 15 MCP tools', async () => {
  ...
  test('generate_script accepts the balance option (melee weapon template, powerful)', async () => {
    const result = await client.call('tools/call', {
      ...
      arguments: {
        type: 'item',
        name: 'BalancedBat',
        properties: { Type: 'Weapon', DisplayName: 'Balanced Bat', category: 'Weapon' },
        balance: 'powerful',
  ...
  test('generate_script supports fixing and sound types', async () => {
    const fixing = await client.call('tools/call', {
      ...
      arguments: {
        type: 'fixing',
        name: 'Fix Bat',
        properties: { require: 'Base.Bat', fixers: [{ material: 'Base.Glue', quantity: 1 }] },
      },
    });
