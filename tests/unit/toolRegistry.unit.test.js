...
describe("tool registry (audit P2)", () => {
  ...
  test("schemas reject invalid input (dispatcher gate)", () => {
    const cases = [
      ...
      [
        "generate_script",
        { type: "not-a-block-type", name: "X", properties: {} },
        true,
      ],
      ...
      ["index_javadocs", { source: "" }, true],
      ["index_javadocs", { source: "/some/javadocs" }, false],
      ["index_javadocs", { source: "/some/javadocs", overwrite: "yes" }, true],
      // source is optional — the repo-shipped distilled markdown is the default
      ["index_javadocs", {}, false],
      ["index_javadocs", { overwrite: false }, false],
      // output without source passes schema (rejected by the handler, not the
      // schema — keeping the schema flat so the deck renders source as optional)
