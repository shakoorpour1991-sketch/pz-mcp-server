...
if CurrentErosionPercentage > 80 then
    for k,v in ipairs(tiles_wallvinescW) do
        overlayMap[v] = {{ name = "other", chance = 4, usage = "", tiles = texture_wallvinescW}}
    end
    for k,v in ipairs(tiles_wallvinescN) do
        overlayMap[v] = {{ name = "other", chance = 4, usage = "", tiles = texture_wallvinescN}}
    end
    for k,v in ipairs(indoor_tiles) do
        overlayMap[v] = {{ name = "other", chance = 3, usage = "", tiles = texture_indoor}}
    end
    for k,v in ipairs(indoor_woodtiles) do
        overlayMap[v] = {{ name = "other", chance = 2, usage = "", tiles = texture_woodindoor}}
    end
    for k,v in ipairs(exterior_indoor_streettiles) do
        overlayMap[v] = {{ name = "other", chance = 10, usage = "", tiles = texture_streets}}
    end
    for k,v in ipairs(indoor_sandwoodtiles) do
        overlayMap[v] = {{ name = "other", chance = 2, usage = "", tiles = texture_sandwood}}
    end
    for k,v in ipairs(tiles_roof1) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof1_array}}
    end
    for k,v in ipairs(tiles_roof2) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof2_array}}
    end
    for k,v in ipairs(tiles_roof5) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof3_array}}
    end
    for k,v in ipairs(tiles_roof6) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof4_array}}
    end
    for k,v in ipairs(tiles_roof7) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof5_array}}
    end        
    for k,v in ipairs(tiles_roof11) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof6_array}}
    end
    for k,v in ipairs(tiles_roof13) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof7_array}}
    end
    for k,v in ipairs(tiles_roof15) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof13_array}}
    end
    for k,v in ipairs(tiles_roof16) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof14_array}}
    end
    for k,v in ipairs(tiles_roof19) do
        overlayMap[v] = {{ name = "other", chance = 5, usage = "", tiles = texture_roof10_array}}
    end
    for k,v in ipairs(tiles_wallvinesW) do
        overlayMap[v] = {{ name = "other", chance = 4, usage = "", tiles = texture_wallvinesW}}
    end
    for k,v in ipairs(tiles_wallvinesN) do
        overlayMap[v] = {{ name = "other", chance = 4, usage = "", tiles = texture_wallvinesN}}
    end
end
if not TILEZED then
